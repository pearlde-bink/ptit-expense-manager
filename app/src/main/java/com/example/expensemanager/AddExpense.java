...
public class AddExpense extends AppCompatActivity {
    ...
    private Category selectedCategory; // Chỉ sử dụng category vừa tạo
    ...
    private List<Category> categoryList; // Danh sách category (chỉ chứa category mới tạo)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        addExpenseButton.setOnClickListener(v -> {
            String expenseTitle = expenseTitleInput.getText().toString().trim();
            String amountStr = amountInput.getText().toString().trim();

            if (expenseTitle.isEmpty() || amountStr.isEmpty()) {
                Toast.makeText(AddExpense.this, getString(R.string.fill_all_fields), Toast.LENGTH_SHORT).show();
                return;
            }

            if (selectedCategory == null) {
                Toast.makeText(AddExpense.this, "Please add a category first", Toast.LENGTH_SHORT).show();
                return;
            }
            ...
            try {
                ...
                if (amount < 0) {
                    Toast.makeText(AddExpense.this, getString(R.string.amount_negative), Toast.LENGTH_SHORT).show();
                    return;
                }
            } catch (NumberFormatException e) {
                Toast.makeText(AddExpense.this, getString(R.string.invalid_amount), Toast.LENGTH_SHORT).show();
                return;
            }
            ...
            try {
                ...
                Toast.makeText(AddExpense.this, "Invalid category ID", Toast.LENGTH_SHORT).show();
                return;
            }

            // Tạo đối tượng Expense mới
            Expense newExpense = new Expense(dateconstructor, expenseTitle, amount, categoryId);

            // Log JSON để kiểm tra
            Gson gson = new Gson();
            Log.d("AddExpense", "Request body: " + gson.toJson(newExpense));

            String token = sharedPreferences.getString("accessToken", "");
            Log.d("AddExpense", "Token before API call: " + token);
            if (token.isEmpty()) {
                Toast.makeText(AddExpense.this, "Please log in to continue", Toast.LENGTH_SHORT).show();
                Intent loginIntent = new Intent(AddExpense.this, AuthenLogin.class);
                startActivity(loginIntent);
                finish();
                return;
            }

            Call<Expense> call = expenseService.createExpense(newExpense, "Bearer " + token);
            call.enqueue(new Callback<Expense>() {
                @Override
                public void onResponse(Call<Expense> call, Response<Expense> response) {
                    Log.d("AddExpense", "Response code: " + response.code());
                    Log.d("AddExpense", "Response message: " + response.message());
                    if (response.code() == 401) {
                        Toast.makeText(AddExpense.this, "Session expired. Please log in again.", Toast.LENGTH_SHORT).show();
                        Intent loginIntent = new Intent(AddExpense.this, AuthenLogin.class);
                        startActivity(loginIntent);
                        finish();
                        return;
                    ...
                    if (response.isSuccessful() && response.body() != null) {
                        ...
                        resultIntent.putExtra("new_expense", response.body());
                        setResult(RESULT_OK, resultIntent);
                        finish();
                    } else {
                        try {
                            Log.e("AddExpense", "Error body: " + response.errorBody().string());
                        } catch (IOException e) {
                            Log.e("AddExpense", "Error reading error body: " + e.getMessage());
                        }
                        Toast.makeText(AddExpense.this, "Failed to add expense: " + response.message(), Toast.LENGTH_SHORT).show();
                    }
                }

                @Override
                public void onFailure(Call<Expense> call, Throwable t) {
                    Log.e("AddExpense", "Network error: " + t.getMessage(), t);
                    Toast.makeText(AddExpense.this, "Network error: " + t.getMessage(), Toast.LENGTH_SHORT).show();
                }
            });
        });
    }
    ...
    private void populateCategoryButtons() {
        ...
        // Chỉ hiển thị category vừa tạo (nếu có)
        if (selectedCategory != null) {
    ...
    private void showAddCategoryDialog() {
        ...
        addButton.setOnClickListener(v -> {
            ...
            // Tạo Category mới
            Category newCategory = new Category(0, 0, title, "EXPENSE", icon);

            // Lấy token
            String accessToken = sharedPreferences.getString("accessToken", "");
            Log.d("AddExpense", "Token in add category: " + accessToken);
            ...
            call.enqueue(new Callback<Category>() {
                ...
                public void onResponse(Call<Category> call, Response<Category> response) {
                    ...
                    if (response.isSuccessful() && response.body() != null) {
                        Toast.makeText(AddExpense.this, "Category added!", Toast.LENGTH_SHORT).show();
                        ...
                        Toast.makeText(AddExpense.this, "Failed to add category: " + response.message(), Toast.LENGTH_SHORT).show();
                    }
                }
